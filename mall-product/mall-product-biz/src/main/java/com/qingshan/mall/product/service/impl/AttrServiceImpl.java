...
//        attrEntity.setAttrName(attr.getAttrName());
        ...
        //2、保存关联关系
        if(attr.getAttrType() == ProductConstants.AttrEnum.ATTR_TYPE_BASE.getCode() && attr.getAttrGroupId()!=null){
    ...
    public PageUtils queryBaseAttrPage(Map<String, Object> params, Long catelogId, String type) {
        QueryWrapper<AttrEntity> queryWrapper = new QueryWrapper<AttrEntity>().eq("attr_type","base".equalsIgnoreCase(type)? ProductConstants.AttrEnum.ATTR_TYPE_BASE.getCode(): ProductConstants.AttrEnum.ATTR_TYPE_SALE.getCode());
    ...
    public AttrRespVo getAttrInfo(Long attrId) {
        ...
        if(attrEntity.getAttrType() == ProductConstants.AttrEnum.ATTR_TYPE_BASE.getCode()){
            //1、设置分组信息
    ...
    public PageUtils getNoRelationAttr(Map<String, Object> params, Long attrgroupId) {
        ...
        //2.2)、这些分组关联的属性
        List<AttrAttrgroupRelationEntity> groupId = relationDao.selectList(new QueryWrapper<AttrAttrgroupRelationEntity>().in(collect.size() > 0,"attr_group_id", collect));
        List<Long> attrIds = groupId.stream().map(item -> {
            return item.getAttrId();
        }).collect(Collectors.toList());

        //2.3)、从当前分类的所有属性中移除这些属性；
        QueryWrapper<AttrEntity> wrapper = new QueryWrapper<AttrEntity>().eq("catelog_id", catelogId).eq("attr_type", ProductConstants.AttrEnum.ATTR_TYPE_BASE.getCode());
        if(attrIds!=null && attrIds.size()>0){
